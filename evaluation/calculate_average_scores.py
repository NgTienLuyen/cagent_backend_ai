...
def main():
    """Hàm chính"""
    print("🚀 Bắt đầu tính toán điểm số trung bình...")
    print("=" * 60)
    ...
    for file_name in evaluation_files:
        ...
        if os.path.exists(file_path):
            ...
            if results:
                ...
                print(f"📁 File: {file_name} - {len(results)} kết quả")
        else:
    ...
    print(f"\n📊 Tổng cộng: {len(all_results)} kết quả đánh giá")
    print("=" * 60)
    ...
    # Lưu kết quả chính
    output_path = "rag_evaluation_average_scores.csv"
    ...
    category_output_path = "rag_evaluation_category_summary.csv"
    save_results_to_csv(category_df, category_output_path)
    
    # In kết quả tóm tắt
    print("\n" + "=" * 60)
    print("🎉 KẾT QUẢ TÍNH TOÁN ĐIỂM SỐ TRUNG BÌNH")
    print("=" * 60)
    
    print(f"📊 Tổng số kết quả: {len(all_results)}")
